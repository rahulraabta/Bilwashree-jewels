...
// Run with: export SANITY_WRITE_TOKEN=your_token; node scripts/migrate_vibes_sanity.js
...
async function migrate() {
  if (!client.config().token) {
    ...
    console.log('Please provide a token with Editor (Write) permissions from sanity.io/manage');
    process.exit(1);
  }

  const products = await client.fetch('*[_type == "product"]{_id, name, category}');
  console.log(`Found ${products.length} products to check for vibe mapping.`);
  ...
  for (const product of products) {
    ...
    // Mapping logic for automatic vibe assignment
  ...
  console.log(`Successfully updated ${patchedCount} products with vibe data.`);
}
